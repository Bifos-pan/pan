...
//ВХОДНЫЕ ДАННЫЕ
const double MASS0 = 70000.0;
const double S_WING = 113.0;
const int ENGINE_COUNT = 2;
const double THRUST_PERCENT = 90.0;

const double H_START = 400.0;
const double H_FINISH = 6500.0;
const double V_START_KMH = 310.0;
const double V_FINISH_KMH = 550.0;
...
bool is_in_flight_envelope(double H, double V_kmh) {
    if (V_kmh < 300.0 || V_kmh > 612.0) return false;
    ...
    if (M > 0.58) return false;

    return true;
}

const double CY0 = -0.085;
...
double thrust_single_d30kp_nominal(double H, double M) {
    ...
    if (H_km <= 0) {
        ...
        altitude_factor = 0.52;
    }
    else {
        altitude_factor = 1.0 - 0.48 * pow(H_km / 11.0, 0.72);
    }

    double mach_factor = 0.9 + 0.22 * M;
...
void create_gnuplot_scripts(const TrajectoryResult& traj_time, const TrajectoryResult& traj_fuel) {
    ...
    comp_script << "set title 'MS-21 Trajectory Comparison (Variant 1)'\n";
    ...
    time_script << "set output 'MS-21_min_time.png'\n\n";
    time_script << "set title 'MS-21 - Minimize Time'\n";
    ...
    fuel_script << "set output 'MS-21_min_fuel.png'\n\n";
    fuel_script << "set title 'MS-21 - Minimize Fuel'\n";
    ...
    bat_file << "echo - MS-21_min_time.png\n";
    bat_file << "echo - MS-21_min_fuel.png\n";
    bat_file << "echo.\n";
    bat_file << "pause\n";
    bat_file.close();
}
...
int main() {
    ...
    cout << "\n=================================================\n";
    cout << "   OPTIMIZACIA TRAEKTORII MS-21 (Variant 10)\n";
    cout << "=================================================\n";
    cout << "Samolyet: MS-21 (massa " << MASS0 / 1000.0 << " t)\n";
    cout << "Dvigateli: 2 x PD-14 (" << THRUST_PERCENT << "% nominala)\n";
    ...
    if (choice == 1) {
        ...
        gp_script << "set output 'MS-21_single.png'\n\n";
        gp_script << "set title 'MS-21 - Minimize Time'\n";
        ...
        gp_script << "set terminal pngcairo size 800,600 enhanced\n";
        gp_script << "set output 'MS-21_single.png'\n\n";
        gp_script << "set title 'MS-21 - Minimize Fuel'\n";
        gp_script << "set xlabel 'Speed V (km/h)'\n";
        gp_script << "set ylabel 'Altitude H (m)'\n";
        gp_script << "set grid\n";
        gp_script << "set datafile separator ','\n";
